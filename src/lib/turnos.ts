...
/**
 * Carga una venta que nadie anotó, en un turno que ya cerró.
 *
 * El caso típico: al cerrar falta un agua y sobran $1.000. No es un faltante,
 * es una venta que no se cargó. La hora tiene que caer adentro del turno.
 *
 * No baja el stock —el faltante ya está contado en el cierre, descontarlo otra
 * vez sería contar dos veces la misma agua— pero sí rehace lo que el turno
 * esperaba, así las dos diferencias que delataron el olvido se cierran solas.
 */
export async function agregarVentaOlvidada(
  turnoId: string,
  datos: {
    alumnoId: string;
    productoId: string;
  ...
  const supabase = await createClient();
  const { error } = await supabase.rpc("agregar_venta_olvidada", {
    p_turno_id: turnoId,
    p_alumno_id: datos.alumnoId,
    p_producto_id: datos.productoId,
    p_creado_en: datos.creadoEn,
    p_cantidad: datos.cantidad ?? 1,
    p_efectivo: datos.efectivo ?? 0,
    p_transferencia: datos.transferencia ?? 0,
  });
  if (error) return { error: error.message };
  revalidatePath("/turnos");
  revalidatePath("/mostrador");
