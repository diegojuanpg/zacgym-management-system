...
function runDaily() {
  ...
  const t0 = Date.now();
  const HARD_LIMIT = 6 * 60 * 1000;       // tope duro de Apps Script
  const RESERVE_REBUILD = 45 * 1000;      // dejar margen para que rebuild corra
  const errores = [];

  const step = function (name, fn) {
    try { fn(); log_(runId, name, 'info', 'ok'); }
    catch (e) { errores.push(name + ': ' + e.message); log_(runId, name, 'error', e.message); }
  };

  step('syncCheckins', syncCheckins);
  step('syncMembers', syncMembers);
  // dias corta a tiempo (deadline compartido) para que rebuildTracking siempre alcance.
  // Lo que no termine se completa en la corrida siguiente (skip por modifiedTime).
  step('syncDiasEntrenamiento', function () {
    syncDiasEntrenamiento(t0 + HARD_LIMIT - RESERVE_REBUILD);
  });
...
function installDailyTrigger() {
  const HORAS = [3, 15]; // dos corridas diarias (3am y 3pm). Cambia a gusto.
  ScriptApp.getProjectTriggers()
    .filter(function (t) { return t.getHandlerFunction() === 'runDaily'; })
    .forEach(function (t) { ScriptApp.deleteTrigger(t); });
  HORAS.forEach(function (h) {
    ScriptApp.newTrigger('runDaily').timeBased().atHour(h).everyDays(1).create();
  });
  Logger.log('Triggers instalados a las ' + HORAS.join('h y ') + 'h (hora del proyecto). '
    + 'Verifica el huso: Config del proyecto > America/Argentina/Buenos_Aires.');
}
